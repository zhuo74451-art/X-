...
def _existing_call_count(out_root: Path) -> int:
    if not out_root.exists():
        return 0
    n = 0
    for p in out_root.rglob("*_response_raw.json"):
        try:
...
def _route_personas(event: dict[str, Any]) -> dict[str, Any]:
    et = str(event.get("event_type") or "").lower()
    title = str(event.get("title") or "").lower()
    summary = str(event.get("summary") or "").lower()
    text = et + " " + title + " " + summary

    if any(k in text for k in ["sanction", "ofac", "lawsuit", "compliance", "enforcement", "regulation", "case_data_regulation"]):
        return {
            "route": "DOWNGRADE_TO_ARTICLE_OR_NEWS",
            "reason": "legal/compliance/sanction content is better suited to article/news format",
            "personas": [],
        }

    if any(k in text for k in ["whale", "on-chain", "onchain", "lookonchain", "arkham", "misinformation", "rumor", "transfer"]):
        return {"route": "X_PERSONA_GENERATION", "personas": ["personal_sharp", "reply_hot_take"], "primary": "personal_sharp"}

    if any(k in text for k in ["etf", "sec", "macro", "inflation", "cpi", "rate"]):
        return {"route": "X_PERSONA_GENERATION", "personas": ["personal_balanced", "reply_hot_take"], "primary": "personal_balanced"}

    if any(k in text for k in ["exchange", "fee", "trading", "market_structure", "industry_structure", "product", "order"]):
        return {
            "route": "X_PERSONA_GENERATION",
            "personas": ["personal_sharp", "personal_balanced", "reply_hot_take"],
            "primary": "personal_sharp",
        }
...
def main() -> None:
    ...
    in_jsonl = root / "data" / "real_event_pack_v006.jsonl"
    ...
    if not load_openrouter_api_key():
        ...
        _write_json(reports_dir / "x_v2_006_real_event_persona_dryrun_report.json", report)
        (reports_dir / "x_v2_006_real_event_persona_dryrun_report.md").write_text(
            "# X v2-006 Real Event Persona Dryrun Report\n\n- status: blocked\n- blocked_reason: BLOCKED_MISSING_OPENROUTER_KEY\n",
            encoding="utf-8",
        )
        raise SystemExit(2)
    ...
    events = _read_jsonl(in_jsonl, limit=5)
    if len(events) < 3:
        report = {
            "task_id": "x_v2_006_real_event_persona_dryrun",
            "generated_at_utc": _utc_now_iso(),
            "status": "BLOCKED_NO_REAL_EVENT_SOURCE",
            "blocked_reason": "real_event_count<3",
            "model_calls_made": 0,
        }
        _write_json(reports_dir / "x_v2_006_real_event_persona_dryrun_report.json", report)
        (reports_dir / "x_v2_006_real_event_persona_dryrun_report.md").write_text(
    ...
    out_root = root / "out" / "x_review_pack_v006"
    out_root.mkdir(parents=True, exist_ok=True)

    existing_calls = _existing_call_count(out_root)
    model_calls = 0
    ...
    for ev in events:
        ...
        if route.get("route") == "DOWNGRADE_TO_ARTICLE_OR_NEWS":
            payload = {
                ...
                "source_mode": str(ev.get("source_mode") or ""),
                "observed_at": str(ev.get("observed_at") or ""),
            }
            _write_json(out_dir / "persona_writer_result.json", payload)
            items.append(
                {
                    "event_id": event_id,
                    "route": "DOWNGRADE_TO_ARTICLE_OR_NEWS",
                    "reason": payload["reason"],
                    "event_meta": {"source_mode": payload["source_mode"], "observed_at": payload["observed_at"], "title": str(ev.get("title") or "")},
                }
            )
            continue
            ...
            items.append(
                {
                    "event_id": event_id,
                    "route": "DOWNGRADE_TO_ARTICLE_OR_NEWS",
                    "reason": "active_events_limit_reached",
                    "event_meta": {"source_mode": str(ev.get("source_mode") or ""), "observed_at": str(ev.get("observed_at") or ""), "title": str(ev.get("title") or "")},
                }
            )
            continue

        personas = route.get("personas") if isinstance(route.get("personas"), list) else []
        system_prompt = _build_persona_system_prompt(sharp=sharp_prompt, balanced=balanced_prompt, reply=reply_prompt, personas=personas)
        payload = {"event_pack": ev, "route": route}

        writer_req = {
            "task_type": "x_persona_split_writer_v006",
            "runtime": "openrouter",
            "model": model,
            "temperature": 0.6,
            "max_tokens": 1000,
            "event_id": event_id,
            "route": route,
            "event_meta": {"source_mode": str(ev.get("source_mode") or ""), "observed_at": str(ev.get("observed_at") or "")},
        }
        _write_json(out_dir / "persona_writer_request.json", writer_req)

        writer_raw: dict[str, Any] = _read_json(out_dir / "persona_writer_response_raw.json") if (out_dir / "persona_writer_response_raw.json").exists() else {}
        persona_result: dict[str, Any] = _read_json(out_dir / "persona_writer_result.json") if (out_dir / "persona_writer_result.json").exists() else {"event_id": event_id, "route": "X_PERSONA_GENERATION"}

        if not (out_dir / "persona_writer_result.json").exists():
            if existing_calls + model_calls + 1 > 12:
        ...
        reviewer_raw: dict[str, Any] = _read_json(out_dir / "ai_reviewer_response_raw.json") if (out_dir / "ai_reviewer_response_raw.json").exists() else {}
        reviewer_result: dict[str, Any] = _read_json(out_dir / "ai_reviewer_result.json") if (out_dir / "ai_reviewer_result.json").exists() else {"event_id": event_id, "review_decision": "NEED_REWRITE"}
        if not (out_dir / "ai_reviewer_result.json").exists():
            if existing_calls + model_calls + 1 <= 12:
                rr = call_llm_task(
                    task_type="x_ai_reviewer_v003",
                    system_prompt=reviewer_prompt,
                    user_prompt=_review_user_prompt(ev, writer_for_review),
                    expect_json=True,
                    temperature=0.2,
                    max_tokens=900,
                )
        ...
        risk_raw: dict[str, Any] = _read_json(out_dir / "ai_risk_response_raw.json") if (out_dir / "ai_risk_response_raw.json").exists() else {}
        risk_result: dict[str, Any] = _read_json(out_dir / "ai_risk_result.json") if (out_dir / "ai_risk_result.json").exists() else {"event_id": event_id, "risk_decision": "NEED_FIX", "risk_level": "medium"}
        if not (out_dir / "ai_risk_result.json").exists():
            if existing_calls + model_calls + 1 <= 12:
                rk = call_llm_task(
        ...
        final = {
            ...
            "route": "X_PERSONA_GENERATION",
            "personas": personas,
            "primary_persona": str(route.get("primary") or ""),
            "status": status,
            "reasons": reasons,
            "score": _safe_int(reviewer_result.get("x_taste_score"), 0),
            "threshold": 80,
            "decision": str(reviewer_result.get("review_decision") or ""),
        ...
        if (out_dir / "x_final_decision.json").exists():
            final = _read_json(out_dir / "x_final_decision.json")
        else:
            _write_json(out_dir / "x_final_decision.json", final)

        if not (out_dir / "x_review_packet.md").exists():
            md: list[str] = []
            md.append("# X Review Packet v006\n\n")
        ...
        items.append(
            {
                ...
                "final": final,
                "event_meta": {"source_mode": final.get("source_mode"), "observed_at": final.get("observed_at"), "title": final.get("title")},
            }
        )
        active_events += 1
    ...
    rejected = [x for x in active if (x.get("final") or {}).get("status") == "REJECTED_BY_REVIEWER"]
    ...
    report = {
        ...
        "status": "DONE",
        "model": model,
        "model_calls_made": existing_calls + model_calls,
        "model_calls_made_this_run": model_calls,
        "generated_events": len(items),
        "counts": {
            "final_approved_count": len(approved),
            "need_rewrite_count": len(need_rw),
            "downgraded_count": len(downgraded),
            "blocked_by_risk": len(blocked),
            "rejected_by_reviewer": len(rejected),
        },
        "items": items,
    ...
    md: list[str] = []
    md.append("# X v2-006 Real Event Persona Dryrun Report\n\n")
    ...
    md.append(f"- generated_events: {report.get('generated_events')}\n")
    ...
    md.append(f"- downgraded_count: {report['counts']['downgraded_count']}\n")
    md.append(f"- blocked_by_risk: {report['counts']['blocked_by_risk']}\n")
    (reports_dir / "x_v2_006_real_event_persona_dryrun_report.md").write_text("".join(md), encoding="utf-8")
